"""fixbug cloud client — stdlib only, zero extra dependencies.

Credentials stored in ~/.fixbug/config.json
...
CLOUD_URL = os.environ.get("FIXBUG_CLOUD_URL", "http://111.231.112.127:8765")
CONFIG_PATH = Path.home() / ".fixbug" / "config.json"
_TIMEOUT = 15
...
def get_api_key() -> Optional[str]:
    ...
    return os.environ.get("FIXBUG_API_KEY") or _load_config().get("api_key")
...
def cmd_whoami() -> dict:
    ...
    token = get_token()
    if not token:
        raise CloudError(0, "not logged in — run: fixbug . --login")
...
def cmd_keygen(note: str = "") -> dict:
    """Generate a fixbug API key (requires login)."""
    token = get_token()
    if not token:
        raise CloudError(0, "not logged in — run: fixbug . --login")
...
def cmd_set_api_key(key: str) -> None:
    """Manually set a fixbug API key in local config."""
    cfg = _load_config()
    cfg["api_key"] = key
    _save_config(cfg)
...
def cmd_sync(store_path: Path, project_name: str = "", pull: bool = True) -> dict:
    ...
    wl_file = fixbug_dir / "memory.json"
    world_lines: dict = {}
    if wl_file.exists():
        try:
...
def interactive_register() -> None:
    ...
    try:
        ...
        print("You're all set. Run: fixbug . to start scanning.")
    except CloudError as e:
