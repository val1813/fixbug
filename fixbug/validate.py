...
def _reset(root: Path):
    shutil.rmtree(root / ".fixbug", ignore_errors=True)


def _run_internal_validation() -> dict:
    results = []
    with tempfile.TemporaryDirectory(prefix="fixbug_validate_") as tmp:
        demo = Path(tmp) / "demo"
        ...
        results.append(("11", advisory["schema"] == "fixbug-advisory-v1" and primary["tone"] == "suggestion", f"confidence={primary['confidence']:.3f}"))
...
def main(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(prog="python -m fixbug.validate")
